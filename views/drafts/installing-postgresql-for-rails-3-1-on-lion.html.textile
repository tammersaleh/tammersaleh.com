...
I recently tried to generate a new Rails 3.1 application based on PostgreSQL on a fresh installation of Lion, and ran into a nasty permission denied issue.  After piecing together the full solution, I thought a blog post might be helpful for anyone else attempting the same task.  The overview of the problem is that Lion actually ships with a PostgreSQL server, so you have to jump through a couple of hoops if you want to use the one provided by "Homebrew":http://mxcl.github.com/homebrew/.
...
And then creating a new Rails 3.1 application, configured to use the PostgreSQL adapter:
...
I started to see signs of woe when @createuser@ failed:
